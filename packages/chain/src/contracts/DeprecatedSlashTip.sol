...
import {DeprecatedUserRegistry} from "./DeprecatedUserRegistry.sol";
import {DeprecatedTip} from "./DeprecatedTip.sol";

/// @notice DEPRECATED: This is the legacy V1 SlashTip contract. Use the new SlashTip instead.
contract DeprecatedSlashTip is AccessControl {
    bytes32 SLASH_TIP_MANAGER = keccak256("SLASH_TIP_MANAGER");
    string public description;

    DeprecatedUserRegistry public userRegistry;
    DeprecatedTip public tipToken;
    ...
    struct UserWithBalance {
        DeprecatedUserRegistry.User user;
        uint256 balance;
    }
    ...
    constructor(address _admin, address _userRegistry, address _tipToken, string memory _description) {
        ...
        userRegistry = DeprecatedUserRegistry(_userRegistry);
        tipToken = DeprecatedTip(_tipToken);
        ...
        onlyRole(SLASH_TIP_MANAGER)
    {
        ...
        DeprecatedUserRegistry.User memory fromUser = userRegistry.getUser(_fromId);
        ...
        DeprecatedUserRegistry.User memory toUser = userRegistry.getUser(_toId);
    ...
    function leaderboard(uint256 _tokenId) public view returns (UserWithBalance[] memory) {
        DeprecatedUserRegistry.User[] memory users = userRegistry.listUsers();
