...
function App() {
	...
	return (
		<div className="font-jacquard text-6xl md:text-8xl flex-grow flex flex-col">
				...
				<div className="text-left md:text-right">
					{data?.map((user) => (
						<div key={`user-${user.nickname}-${user.balance}`}>
							{user.balance}*/<span className="inline-block md:hidden">{renderUser(user)}</span>
						</div>
					))}
				</div>
			...
			<div className="flex flex-col justify-end items-center gap-y-8 py-8 md:py-12">
				<button
					type="button"
					className="text-4xl md:text-6xl p-2 bg-[#02d100] text-[#03ff00] cursor-not-allowed"
				>
					redeem
				</button>
			</div>
